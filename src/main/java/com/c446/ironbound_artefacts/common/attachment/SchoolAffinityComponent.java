...
public class SchoolAffinityComponent implements INBTSerializable<CompoundTag> {
    ...
    public static SchoolType getSchool(Player player) {
        if (player.hasData(RegistryAttachment.MARKOHESHKIR_ATTACHMENT)) {
            return SchoolRegistry.getSchool(ResourceLocation.parse(player.getData(RegistryAttachment.MARKOHESHKIR_ATTACHMENT).school));
        }
        return null;
    }
